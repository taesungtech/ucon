...
namespace UCON_TEST
{
                ...
                switch ((ModBusReadData.address + i))
                {
                    ...
                    case 0x80: // 
                        ...
                        if (ModBusReadData.stationNumber == 14)
                        {
                            ...
                            digital_output_RD = ((digital_output_RD & 0x0ffffe) | ((ModBusReadData.data[i]) & 0x0001)) & 0xfffff;
                        ...
                        else if (ModBusReadData.stationNumber == 11)
                        {
                            ...
                            digital_output_RD = ((digital_output_RD & 0x0e0001) | ((ModBusReadData.data[i]) << 1)) &0xfffff;
                        }
                        else if (ModBusReadData.stationNumber == 12)
                        {

                            digital_output_RD = ((digital_output_RD & 0x01ffff) | ((ModBusReadData.data[i])<<17)) & 0xfffff;
                        }
    ...
    */
        private void ADC_check1( )
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                if ((analog_intput_RD[i] - 100) > analog_intput_wr[i])
                {
                    analog_intput_ERR1 = analog_intput_ERR1 | (0x001 << i);
                }
                else if ((analog_intput_RD[i] + 100) < analog_intput_wr[i])
                {
                    analog_intput_ERR1 = analog_intput_ERR1 | (0x001 << i);
                }
                else
        ...
        private void ADC_check2()
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                if ((analog_output_RD[i] - 100) > analog_output_wr[i])
                {
                    analog_output_ERR1 = analog_output_ERR1 | (0x001 << i);
                }
                else if ((analog_output_RD[i] + 100) < analog_output_wr[i])
                {
                    analog_output_ERR1 = analog_output_ERR1 | (0x001 << i);
                }
                else
